...
# 2. OPTIONALLY uses team_rosters.csv (if present)
#    - If present, it will be the base for combined_player_view_full.parquet
#      so you get team_key / team_name columns.
#    - If missing, it falls back to league_players.csv (no hard error).
...
#   - Carries through team_name, team_key, etc. from rosters if available.
...
def main():
    ...
    if not os.path.exists("league_players.csv"):
        ...
        lp_df_tmp = pd.read_csv("league_players.csv", dtype=str)
        if "player_name" in lp_df_tmp.columns and len(lp_df_tmp) > 0:
            frac_unknown = (lp_df_tmp["player_name"] == "Unknown").mean()
    ...
    # ----- Choose base_for_combined: team_rosters if present, else league_players -----
    if os.path.exists("team_rosters.csv"):
        print("Using existing team_rosters.csv as base for combined view.")
        try:
            ...
            print("Failed to read team_rosters.csv, falling back to league_players:",
                  type(e).__name__, e)
            base_for_combined = lp_df.copy()
    else:
        print("team_rosters.csv not found – combined view will NOT have team columns.")
        base_for_combined = lp_df.copy()
